...
struct XamarinCallState {
	...
	uint64_t rbp;
	uint64_t rax;
	uint64_t rdx_out; // use this field as a temporary rdx field to store output. It makes the marshalling code a bit easier to have this field just after the rax field..
	long double xmm0;
	long double xmm1;
	long double xmm2;
	long double xmm3;
